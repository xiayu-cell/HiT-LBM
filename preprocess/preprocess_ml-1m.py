...
user_core = 50 # 用户交互行为最小值，小于该值会被过滤
item_core = 0 # item被交互的最小值
attribute_core = 0
...
def ml_1m_meta(meta_file, data_maps):  # return the metadata of products
    ...
    with open(meta_file, 'r', encoding='latin-1') as r:
        ...
        for row in lines:
            ...
            new_info_rec = {'categories': [cate_list[0]],
                        'title': correct_title(title)}
            new_info_llm = {'categories': cate_list,
                        'title': correct_title(title)}
            datas[iid] = new_info_rec
            datas_llm[iid] = new_info_llm

    meta_set = set(datas.keys())
    meta_set_llm = set(datas_llm.keys())
    return datas, datas_llm, item_asins.difference(meta_set)
...
def preprocess(data_file, meta_file, user_file, processed_dir, data_type='ml-1m'):
    ...
    meta_infos,meta_infos_llm, item_diff = ml_1m_meta(meta_file, data_maps)
    ...
    for uid, item_list in user_items.items():
        # 计算划分训练集和测试集的索引
        split_index = int(len(item_list[0]) * 0.9)
        # 划分训练集和测试集
        train_set[uid] = [item_list[0][:split_index],item_list[1][:split_index]]
        test_set[uid] = [item_list[0][split_index:],item_list[1][split_index:]]
        train_test_set[uid] = split_index
    ...
    train_test_split_index = processed_dir + '/train_test_split_index.json'
    ...
    save_json(train_set, train_file)
    save_json(test_set, test_file)
    save_json(train_test_set,train_test_split_index)
