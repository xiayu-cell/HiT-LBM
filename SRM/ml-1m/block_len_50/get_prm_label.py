...
with open('/mmu_nlp_ssd/xiayu12/LIBER_ours_train/PRM/ml-1m/block_len_50/prm_data.json','r',encoding='utf-8') as f:
    prm_data = json.load(f)
prediction_folder = '/mmu_nlp_ssd/xiayu12/LIBER_ours_train/PRM/ml-1m/block_len_50'
all_prediction = {}
total = 0
pos = 0
for filename in os.listdir(prediction_folder):
    if filename.startswith(f'pre_prediction') and filename.endswith(".json"):
...
for item in prm_data:
    if item['block'] in all_prediction[item['user']]:
...
path = '/mmu_nlp_ssd/xiayu12/LIBER_ours_train/PRM/ml-1m/block_len_50/pre_final_prm_data.json'
