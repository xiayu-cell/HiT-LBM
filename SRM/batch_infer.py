...
def run_inference(model_path,batch, bs, thread_id):
    ...
    for item in batch:
        ...
        prompt = 'Based on the user\'s current movie preferences and interest changes, please determine whether the user will like the target movie. Output only Yes or No'
        
        current_sum = '[Current movie preferences and Interest changes]'
        target_item = '[Target Movie]'
        # targets = [targets[0]]
        for t in targets:
            title, category, lb = t
            ti = f'movie: {title}, category: {category}'
            prompt = '\n'.join([prompt,current_sum,cur_sum, target_item, ti])
            messages = [
    ...
    sampling_param = SamplingParams(temperature=0,max_tokens=10,repetition_penalty=1.05,logprobs=20)
    ...
    with open(f'/mmu_nlp_ssd/xiayu12/LIBER_ours_train/PRM/amz/block_len_50/prediction_{thread_id}.json', 'w') as f:
...
if __name__ == "__main__":
    prompt_path = '/mmu_nlp_ssd/xiayu12/LIBER_ours_train/PRM/amz/block_len_50/prm_data.json'
