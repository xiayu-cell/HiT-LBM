...
class EmbLabelDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        根据索引返回一个样本
        """
        # emb = self.embs2[idx] - self.embs1[idx]
...
def train():
    ...
    emb_path = '/mmu_nlp_ssd/xiayu12/LIBER_ours_train/PRM/ml-1m/block_len_50/bge_avg_pre_cur_emb.sum'
    ...
    for epoch in range(num_epochs):
        # 训练阶段
        model.train()
        for i, (batch_embeddings1, batch_labels) in tqdm(enumerate(train_dataloader), total=len(train_dataloader)):
            # 将数据移动到设备
            ...
            # 前向传播
            outputs = model(batch_embeddings1)
        ...
        with torch.no_grad():
            for batch_embeddings1, batch_labels in test_dataloader:
                # 将数据移动到设备
                batch_embeddings1 = batch_embeddings1.to(device)
                # batch_embeddings2 = batch_embeddings2.to(device)
                # batch_embeddings = torch.cat([batch_embeddings1,batch_embeddings2],dim=-1)
