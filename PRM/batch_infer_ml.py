...
def run_inference(model_path,batch, bs, thread_id):
    ...
    for item in batch:
        ...
        prefix = 'this user\'s previous movie interest summary and current movie viewing history are listed below.  Movie viewing history includes the movie title, category, and the user\'s rating. If the user\'s rating is greater than or equal to 4, it is considered that the user likes the movie; if the rating is less than 4, it is considered that the user dislikes the movie.'

        prompt = 'Based on the user\'s watching history, please determine whether the user will like the target movie. Output only Yes or No'
        
        current_sum = '[Current movie watching history]'
    ...
    with open(f'/mmu_nlp_ssd/xiayu12/LIBER_ours_train/PRM_point/ml-1m/prediction_{thread_id}.json', 'w') as f:
...
if __name__ == "__main__":
    prompt_path = '/mmu_nlp_ssd/xiayu12/LIBER_ours_train/PRM_point/ml-1m/prm_data.json'
