    ...
    def _prepare_data(self):
        ...
        for user, blocks in self.label_data.items():
            for block_id, emb in blocks.items():
                embs.append(self.emb_data[user][block_id])
...
def train():
    ...
    emb_path = '/mmu_nlp_ssd/xiayu12/LIBER_ours_train/PRM_point/ml-1m/bge_avg_cur_emb.sum'
    label_path = '/mmu_nlp_ssd/xiayu12/LIBER_ours_train/PRM_point/ml-1m/prm_train.json' 
    threshold = 0.5
    ...
    for epoch in range(num_epochs):
        ...
        # 保存模型权重
        torch.save(model.state_dict(), f'/mmu_nlp_ssd/xiayu12/LIBER_ours_train/PRM_point/ckp/ml-1m/cls_epoch_{epoch}_threshold_{threshold}.pth')
        ...
        results_df.to_csv(f'/mmu_nlp_ssd/xiayu12/LIBER_ours_train/PRM_point/test_ml_prm_epoch_{epoch}.csv')
