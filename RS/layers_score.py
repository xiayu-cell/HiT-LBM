...
from transformer import Decoder_xiayu,Decoder_xiayu2
...
class ConvertNet(nn.Module):
    ...
    def __init__(self, args, inp_dim, dropout, conv_type):
        ...
        elif self.type == 'SA': # 使用 SA进行融合
            print('convert module: SA')
            self.sub_module = SelfAttention(inp_dim, 1)
            self.linear = nn.Linear(inp_dim, args.convert_arch[-1])
        elif self.type == 'MSA': # 使用 MSA进行融合
            print('convert module: MSA')
            self.sub_module = MaskedSelfAttention(inp_dim, 1)
            self.cross = CrossAttention(inp_dim, 8)
            # self.linear = nn.Linear(inp_dim, args.convert_arch[-1])
            self.hist_linear = nn.Linear(inp_dim, args.convert_arch[-1])
            self.item_linear = nn.Linear(inp_dim, args.convert_arch[-1])
        elif self.type == 'GRU':
            print('convert module: GRU')
            self.sub_module = InterestExtractor_xiayu(inp_dim, inp_dim)
            self.cross = CrossAttention(inp_dim, 8)
            # self.linear = nn.Linear(inp_dim, args.convert_arch[-1])
            self.hist_linear = nn.Linear(inp_dim, args.convert_arch[-1])
            self.item_linear = nn.Linear(inp_dim, args.convert_arch[-1])
        elif self.type == 'AIGRU':
            print('convert module: AIGRU')
            self.sub_module = InterestExtractor_xiayu(inp_dim, inp_dim)
            self.cross = InterestEvolving_xiayu(input_size=inp_dim, gru_type='AIGRU')
            # self.linear = nn.Linear(inp_dim, args.convert_arch[-1])
            self.hist_linear = nn.Linear(inp_dim, args.convert_arch[-1])
            self.item_linear = nn.Linear(inp_dim, args.convert_arch[-1])
        elif self.type == 'Decoder':
            print('convert module: Decoder')
            # print('convert module: dim', inp_dim)
            self.sub_module = Decoder_xiayu(inp_dim, 2,8,0.1)
            self.hist_linear = nn.Linear(inp_dim, args.convert_arch[-1])
            self.item_linear = nn.Linear(inp_dim, args.convert_arch[-1])

        elif self.type == 'Decoder_layer':
            print('convert module: Decoder')
            self.sub_module = Decoder_xiayu2(inp_dim, 1,12,0.1)
            self.hist_linear = nn.Linear(inp_dim, args.convert_arch[-1])
            self.item_linear = nn.Linear(inp_dim, args.convert_arch[-1])
    ...
    def forward(self, x_list,orig_item_dens_vec,prm_vector_list):
        if self.type == 'HEA':
            out = self.sub_module(x_list)
        elif self.type == 'SA':
            # Pad sequences to the same length
            padded_embeddings = pad_sequence(x_list, batch_first=True).to(x_list[0].device)  # (batch_size, max_seq_len, embed_dim)
            # Create a mask to ignore padding
            mask = torch.zeros(len(x_list), padded_embeddings.size(1)).to(x_list[0].device)  # (batch_size, max_seq_len)
            for i, emb in enumerate(x_list):
                mask[i, :emb.size(0)] = 1  # Mark valid tokens with 1
            # Apply Self-Attention
            output = self.sub_module(padded_embeddings, mask=mask)  # (batch_size, max_seq_len, embed_dim)
            # Aggregate the output (e.g., take the mean of the valid tokens)
            aggregated_output = (output * mask.unsqueeze(-1)).sum(dim=1) / mask.sum(dim=1).unsqueeze(-1)  # (batch_size, embed_dim)
            out = self.linear(aggregated_output)
            return out
        elif self.type == 'MSA':
            # Pad sequences to the same length
            padded_embeddings = pad_sequence(x_list, batch_first=True).to(x_list[0].device)  # (batch_size, max_seq_len, embed_dim)
            ...
            # Apply Self-Attention
            output = self.sub_module(padded_embeddings, mask=mask)  # (batch_size, max_seq_len, embed_dim)
            output = self.cross(orig_item_dens_vec,output, mask=mask).squeeze(1)  # (batch_size, 1, embed_dim)
            out1 = self.hist_linear(output)
            # out2 = self.item_linear(item_output)
            out2 = self.item_linear(orig_item_dens_vec)
            out = torch.cat([out1, out2], dim=-1)
            return out
        elif self.type == 'GRU':
            padded_embeddings = pad_sequence(x_list, batch_first=True).to(x_list[0].device)  # (batch_size, max_seq_len, embed_dim)
            keys_length = torch.zeros(len(x_list),1).to(x_list[0].device)
            # print(padded_embeddings.shape)
            for i, emb in enumerate(x_list):
                # print(emb.size(0))
                keys_length[i] = emb.size(0)
            output = self.sub_module(padded_embeddings, keys_length)
            ...
            output = self.cross(orig_item_dens_vec,output, mask=mask).squeeze(1)  # (batch_size, 1, embed_dim)
            # avg pooling
            # hist_output = (output * mask.unsqueeze(-1)).sum(dim=1) / mask.sum(dim=1).unsqueeze(-1)  # (batch_size, embed_dim)
            # item_output = (output * item_mask.unsqueeze(-1)).sum(dim=1)
            # out = self.linear(aggregated_output)
            out1 = self.hist_linear(output)
            # out2 = self.item_linear(item_output)
            out2 = self.item_linear(orig_item_dens_vec)
            ...
            output = self.cross(orig_item_dens_vec,output,keys_length, mask=mask.unsqueeze(-1))  # (batch_size, 1, embed_dim)
            out1 = self.hist_linear(output)
            out2 = self.item_linear(orig_item_dens_vec)
            out = torch.cat([out1, out2], dim=-1)
            return out
            ...
            # Create a mask to ignore padding
            mask = torch.zeros(len(x_list), padded_embeddings.size(1)).to(x_list[0].device)  # (batch_size, max_seq_len)
            # src_mask = torch.ones(len(x_list), padded_embeddings.size(1)).to(x_list[0].device)
            for i, emb in enumerate(x_list):
                mask[i, :emb.size(0)] = 1  # Mark valid tokens with 1
            ...
            causal_mask = torch.triu(torch.ones(padded_embeddings.size(1), padded_embeddings.size(1),device=padded_embeddings.device), diagonal=1).bool().unsqueeze(0).unsqueeze(1) # (1, 1, seq_len, seq_len)

            # Apply Self-Attention
            # scr_mask不需要，因为 cross attention的k和v shape为 1xd
            output = self.sub_module(padded_embeddings,orig_item_dens_vec,src_mask=None, trg_mask=causal_mask).squeeze(1)  # (batch_size, max_seq_len, embed_dim)
            out1 = self.hist_linear(output)
            out2 = self.item_linear(orig_item_dens_vec)
            out = torch.cat([out1, out2], dim=-1)
            return out
            ...
            # Pad sequences to the same length
            padded_embeddings = pad_sequence(x_list, batch_first=True).to(x_list[0].device)  # (batch_size, max_seq_len, embed_dim)
            prm_vector = pad_sequence(prm_vector_list, batch_first=True).to(x_list[0].device)
            # Create a mask to ignore padding
            mask = torch.zeros(len(x_list), padded_embeddings.size(1)).to(x_list[0].device)  # (batch_size, max_seq_len)
            # src_mask = torch.ones(len(x_list), padded_embeddings.size(1)).to(x_list[0].device)
            for i, emb in enumerate(x_list):
            ...
            if torch.isinf(padded_embeddings).any() or torch.isnan(padded_embeddings).any():
                print(padded_embeddings)
                exit(0)
            # Apply Self-Attention
            # scr_mask不需要，因为 cross attention的k和v shape为 1xd
            # padded_embeddings = padded_embeddings * prm_vector
            output = self.sub_module(padded_embeddings,orig_item_dens_vec,src_mask=None, trg_mask=causal_mask)  # (batch_size, max_seq_len, embed_dim)
            # Aggregate the output (e.g., take the mean of the valid tokens)
            # print(output.shape)
            # print(prm_vector.shape)
            # print(prm_vector)
            # output = output * prm_vector
            # prm_vector = prm_vector / prm_vector.sum(dim=1).unsqueeze(-1)
            aggregated_output = (output * mask.unsqueeze(-1)).sum(dim=1) / mask.sum(dim=1).unsqueeze(-1)  # (batch_size, embed_dim)
            # final_out = torch.cat([aggregated_output, orig_item_dens_vec], dim=-1)
            # aggregated_output = (output * prm_vector).sum(dim=1)
            out1 = self.hist_linear(aggregated_output)
            out2 = self.item_linear(orig_item_dens_vec)
            out = torch.cat([out1, out2], dim=-1)
            ...
            hist_output = []
            for i, emb in enumerate(x_list):
                hist_output.append(torch.mean(emb,dim=0).unsqueeze(0))
            hist_output = torch.cat(hist_output,dim=0)
            out1 = self.hist_linear(hist_output)
            # out2 = self.item_linear(item_output)
            out2 = self.item_linear(orig_item_dens_vec)
            out = torch.cat([out1, out2], dim=-1)
            return out
...
class MaskedSelfAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        causal_mask = torch.triu(torch.ones(seq_len, seq_len, device=x.device), diagonal=1).bool().unsqueeze(0).unsqueeze(1) # (1, 1, seq_len, seq_len)
        
        # Combine causal mask with padding mask if provided
        # if mask is not None:
        #     # Padding mask: (batch_size, 1, 1, seq_len)
        #     padding_mask = mask.unsqueeze(1).unsqueeze(2).bool().to(x.device)
        #     # Combine masks: (batch_size, num_heads, seq_len, seq_len)
        #     # print("padding_mask",padding_mask[0])
        #     # print("causal_mask",causal_mask)
        #     combined_mask = padding_mask & ~causal_mask  # Apply causal mask and padding mask
            # print("combined_mask",combined_mask[0])
        ...
        weighted_values = torch.matmul(attn_weights, V)  # (batch_size, num_heads, seq_len, head_dim)
        weighted_values = weighted_values.transpose(1, 2).contiguous().view(batch_size, seq_len, embed_dim)  # (batch_size, seq_len, embed_dim)
        
        # Output projection
        output = self.W_o(weighted_values)  # (batch_size, seq_len, embed_dim)
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, e_outputs, mask=None):
        """
        ...
        """
        x = x.unsqueeze(1)
        batch_size, seq_len, embed_dim = e_outputs.size()
        
        # Compute Query, Key, Value
        Q = self.W_q(x)  # (batch_size, 1, embed_dim)
        K = self.W_k(e_outputs)  # (batch_size, seq_len, embed_dim)
        V = self.W_v(e_outputs)  # (batch_size, seq_len, embed_dim)
        ...
        if mask is not None:
            mask = mask.unsqueeze(1).unsqueeze(2).to(x.device) # (batch_size, 1, seq_len)
        ...
        # Compute weighted values
        weighted_values = torch.matmul(attn_weights, V)  # (batch_size, num_heads, 1, head_dim)
        weighted_values = weighted_values.transpose(1, 2).contiguous().view(batch_size, 1, embed_dim)  # (batch_size, seq_len, embed_dim)
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        # Apply mask (if provided)
        if mask is not None:
            mask = mask.unsqueeze(1).unsqueeze(2).to(x.device) # (batch_size, 1, 1, seq_len)
            scores = scores.masked_fill(mask == 0, float('-inf'))
...
def process_embeddings(embeddings_list, embed_dim, num_heads):
    """
    ...
    """
    # Pad sequences to the same length
    padded_embeddings = pad_sequence(embeddings_list, batch_first=True)  # (batch_size, max_seq_len, embed_dim)
    
    # Create a mask to ignore padding
    mask = torch.zeros(len(embeddings_list), padded_embeddings.size(1))  # (batch_size, max_seq_len)
    for i, emb in enumerate(embeddings_list):
        mask[i, :emb.size(0)] = 1  # Mark valid tokens with 1
    
    # Apply Self-Attention
    attention_layer = SelfAttention(embed_dim, num_heads)
    output = attention_layer(padded_embeddings, mask=mask)  # (batch_size, max_seq_len, embed_dim)
...
class InterestExtractor_xiayu(nn.Module):
    """
    Interest extractor in DIEN
    """
    def __init__(self, input_size, hidden_size):
        super(InterestExtractor_xiayu, self).__init__()
...
class InterestEvolving_xiayu(nn.Module):
    """
    Interest evolving in DIEN
    """

    def __init__(self, input_size, gru_type='AIGRU', dropout=0):
        super(InterestEvolving_xiayu, self).__init__()
...
if __name__ == '__main__':
    model = InterestEvolving_xiayu(input_size=768, gru_type='AIGRU')
    # query:       [btz, 1, hdsz]
    # keys:        [btz, seq_len ,hdsz]
    # keys_length: [btz, 1]
    query = torch.randn(3,768)
    keys = torch.randn(3,7,768)
    keys_length = torch.Tensor([[5],[6],[7]])
    print(keys_length.shape)
    o = model(query,keys,keys_length)
    print(o.shape)
