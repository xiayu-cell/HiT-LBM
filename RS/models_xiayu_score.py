...
from layers_score import AttentionPoolingLayer, MLP, CrossNet, ConvertNet, CIN, MultiHeadSelfAttention, \
...
from layers_score import Phi_function
...
class BaseModel(nn.Module):
    def __init__(self, args, dataset):
        ...
        self.score_embedding =  nn.Embedding( 10, self.embed_dim)

        if self.augment_num:
            self.convert_module = ConvertNet(args, self.dense_dim + self.embed_dim, self.convert_dropout, self.convert_type)
    ...
    def process_input(self, inp):
        ...
        if self.task == 'ctr':
            ...
            if self.prm:
                ...
                scores  = prm_vector.squeeze(-1)
                scores = torch.log(scores) + math.log(1000)
                scores = scores.long()
                bs, n = scores.shape
                score_emb = self.score_embedding(scores.to(device)).view(-1, n, self.embed_dim)
                item_score = (torch.ones(bs,1)*9).long()
                item_score_emb = self.score_embedding(item_score.to(device)).view(-1, self.embed_dim)
            ...
            if self.augment_num:
                ...
                orig_dens_vec_len = inp['hist_aug_vec_len'].to(device)
                orig_item_dens_vec = inp['item_aug_vec'].to(device)
                orig_item_dens_vec = torch.cat([orig_item_dens_vec,item_score_emb],dim=-1)
                orig_dens_vec = inp['hist_aug_vec'].to(device)
                ...
                for i in range(bs):
                    ...
                    # 将结果添加到列表中
                    actual_score_vec = score_emb[i,:length,:]
                    actual_dens_vec = torch.cat([actual_dens_vec,actual_score_vec],dim=-1)
...
class DeepInterestNet(BaseModel):
    ...
    def forward(self, inp):
        ...
        mlp_out = self.final_mlp(concat_input) # n+m -> n+m
        logits = self.final_fc(mlp_out)        # n+m -> 1
